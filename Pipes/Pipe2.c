...
int main(void)
{
int pfd[2]; char fdstr[10]; int status;
/* Creación del pipe */
if(pipe(pfd) == -1)
  perror("pipe");
...
/* Cierre del descriptor de escritura del pipe en el proceso hijo */
  if(close(pfd[1]) == -1)
    perror("close");
  /* int sprintf(char *buffer, const char *formato,...): su funcionamiento es idéntico
   a la función printf, salvo que la salida generada se pone en el array apuntado
   por buffer. El valor devuelto es igual al número de caracteres puesto en el array */
  sprintf(fdstr,"%d",pfd[0]);
  /* int execlp(const char *filename, const char *argv[0],..): permite la ejecución
  del programa filename como un nuevo proceso imagen. La lista de argumentos es:
  const char *filename: contiene el nombre del programa ejecutable.
  const char argv[]: el primer elemento de array es el nombre de programa, y el
  último un puntero NULL. */
  if (execlp("./ReceptorPipe","p3-3", fdstr, NULL) == -1)
      {
